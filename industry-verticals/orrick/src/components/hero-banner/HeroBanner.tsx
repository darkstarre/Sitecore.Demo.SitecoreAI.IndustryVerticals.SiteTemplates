...
import { ImageField, useSitecore, withDatasourceCheck } from '@sitecore-content-sdk/nextjs';
...
type SitecoreRenderingNode = {
  componentName?: string;
  placeholders?: Record<string, SitecoreRenderingNode[]>;
};

const hasFeaturedArticlesRendering = (nodes: SitecoreRenderingNode[] = []): boolean => {
  return nodes.some((node) => {
    if (node.componentName === 'FeaturedArticles') return true;

    const childGroups = Object.values(node.placeholders || {});
    return childGroups.some((children) => hasFeaturedArticlesRendering(children || []));
  });
};
...
export const DefaultHeroBanner = (props: HeroBannerProps) => {
  ...
  const topLevelRenderings = Object.values(
    page?.layout?.sitecore?.route?.placeholders || {}
  ).flat();
  const shouldRenderInlineFallback = !hasFeaturedArticlesRendering(
    topLevelRenderings as SitecoreRenderingNode[]
  );
  ...
  return (
    <>
      ...
      {shouldRenderInlineFallback ? <InlineFeaturedArticles /> : null}
    </>
  );
};
