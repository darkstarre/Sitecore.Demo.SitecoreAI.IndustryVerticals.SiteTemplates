...
Hussmann is a demo website scaffold based on Forma Lux, prepared for an industrial refrigeration experience. It includes the same multisite foundations, localization support, and content SDK wiring so the team can focus on Hussmann-specific branding and content.
...
2. Starting from the root of the repository navigate to site app folder
   `cd industry-verticals\hussmann`
3. Copy the environment file `.env.local.example`
...
6. Install dependencies:
   from `industry-verticals\hussmann` run `npm install`
