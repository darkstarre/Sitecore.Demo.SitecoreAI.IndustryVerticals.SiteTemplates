...
import { UCHICAGOMED_SITE_NAME } from 'src/constants/site';
...
export const getStaticPaths: GetStaticPaths = async (context) => {
  ...
  if (process.env.NODE_ENV !== 'development' && scConfig.generateStaticPaths) {
    try {
      paths = await client.getPagePaths([UCHICAGOMED_SITE_NAME], context?.locales || []);
    } catch (error) {
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  if (context.preview && isDesignLibraryPreviewData(context.previewData)) {
    ...
    page = context.preview
      ...
      : await client.getPage(path, { locale: context.locale, site: UCHICAGOMED_SITE_NAME });
  }
  if (page) {
    props = {
      page,
      dictionary: await client.getDictionary({
        site: UCHICAGOMED_SITE_NAME,
        locale: page.locale,
      }),
